...
void CSprite::DrawFlip(float x, float y, int spritewidth, int spriteheight)
{
	...
	// Scale the sprite negatively along the x-axis to flip it horizontally
	D3DXMatrixScaling(&this->matScaling, -1.0f * (FLOAT)spritewidth, (FLOAT)spriteheight, 1.0f);
	D3DXMatrixTranslation(&matTranslation, x + (FLOAT)spritewidth - cx, g->GetBackBufferHeight() - y + cy, 0.1f);
